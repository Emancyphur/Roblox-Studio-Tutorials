...
local TeamCharacters = ServerStorage:WaitForChild("Multiple TeamCharacters")
...
local function AnimatorCreator()
	...
	for i,Character in pairs(TeamCharacters:GetDescendants()) do
...
AnimatorCreator()
TeamCharacters.DescendantAdded:Connect(function(Item)
...
local function LoadCharacter(player,NewTeam)
	...
	elseif RigTypes[player] ~= nil then
		...
		elseif player.Team and player.Neutral == false then
			...
			local FindTeamCharacterFolder = TeamCharacters:FindFirstChild(player.Team.Name..RigType)
			
			if not FindTeamCharacterFolder then player:LoadCharacter() warn("No TeamCharacters Folder found for "..player.Team.Name)

			elseif FindTeamCharacterFolder then
			
				local CharacterFolder = FindTeamCharacterFolder:GetChildren()
				local FindTeamCharacter = CharacterFolder[math.random(1,#CharacterFolder)]
